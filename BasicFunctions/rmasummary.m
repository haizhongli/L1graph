...
%   'log', 'log2' (default), 'log10', 'natural' scales, or a function
...
% $Revision: 1.1.10.8 $   $Date: 2008/06/16 16:34:02 $
...
if ~isnumeric(values) || ~isreal(values)
   error('Bioinfo:rmasummary:ProbeIntensityNotNumericAndReal',...
         'Probe intensity values must be numeric and real.') 
end

if ~isnumeric(indices) || ~isreal(indices) || ~isvector(indices)...
        || any(indices < 0) || any(isnan(indices))
   error('Bioinfo:rmasummary:InvalidProbeIndices',...
         'Indices must be a numeric, real and positive vector.') 
end
...
if numel(indices) ~= size(values,1)
   error('Bioinfo:rmasummary:NotEqualNumberOfProbes',...
         'The probe indices vector must have the same number of rows of probe intensity matrix.')
end
            ...
            switch(k)
                case 1  % output type
                    ...
                    if ischar(outputtype)
                        okmethods = {'log','log2','log10','natural'};
                        output = strmatch(lower(outputtype), okmethods, 'exact');
                        if isempty(output)
                            error('Bioinfo:rmasummary:UnknownOutputType',...
                                'Unknown output type: %s.',pval);
                        elseif length(output)>1
                            error('Bioinfo:rmasummary:AmbiguousOutputType',...
                                'Ambiguous output type: %s.',pval);
                        else
                            switch output
                                case 1
                                    outputFun = @log;
                    ...
                    elseif isa(outputtype, 'function_handle')
                        ...
                        error('Bioinfo:rmasummary:OutputTypeNotFunctionHandle',...
                            'OUTPUT must be a string or a function handle.');
                    end
            end
        end
    end
...
if output == 2
    ...
    try
        if output ~= 4 % Convert back to natural scale first
            sumData = feval(@pow2, sumData);
        end
        ...
        warning('Bioinfo:rmasummary:OutputTypeFunctionErrored',...
            '%s \nThe output is in the default log2 scale instead.',theException.message);
    end
end
end % rmasummary
