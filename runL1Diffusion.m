...
sigma = 0.4; %% paramter for guassian kernel.
...
%% spectral clsutering of Gaussian similarity

idx_g = spectralClustering(adj_gaussian,num_cluster);

res = bestMap(true_labels,idx_g);
%=============  evaluate AC: accuracy ==============
AC = length(find(true_labels == res))/length(true_labels);
%=============  evaluate MIhat: nomalized mutual information =================
MIhat = MutualInfo(true_labels,res);

sprintf('Gaussian NMI is:%.4f\n',MIhat)
sprintf('Gaussian ACC is: %.4f\n', AC)
